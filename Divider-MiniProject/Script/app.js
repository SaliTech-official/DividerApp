...
LABELS.forEach((item, idx) => {
    ...
    div.innerHTML = `
        <div class="label-row">
            ...
            <input type="number" min="0" step="1" id="count-${idx}" class="label-input" placeholder="تعداد" />
        </div>
...
document.getElementById('calculate-btn').addEventListener('click', () => {
    ...
    const options = LABELS.map((item, idx) => {
        ...
        const count = parseInt(document.getElementById(`count-${idx}`).value) || 0;
        const exclude = document.getElementById(`exclude-${idx}`).checked;
        return {
            value: item.value,
            unlimited,
            count: unlimited ? Infinity : count,
            exclude
        };
    ...
    LABELS.forEach((item, idx) => {
        ...
        if (exclude) {
            ...
            html += `<div>${item.label}: <span class="font-bold">${result.used[usedIdx]}</span></div>`;
            usedIdx++;
        }
    });
